...
create table reservations (
    ...
    res_emprunteur integer not null references emprunteurs(emp_code) on delete cascade,
    res_livre varchar(4) not null references livres (liv_code) on delete cascade,
    res_date date,
    unique (res_livre, res_date)
)
;

drop trigger if exists tg_set_reservations_default_date_if_null;
delimiter //
create trigger tg_set_reservations_default_date_if_null
before insert on reservations for each row
begin
    if (new.res_date is null) then
        set new.res_date = current_date;
    end if;
end
//
delimiter ;
...
create table livres_auteurs (
    ...
    auteur_id integer not null references auteurs(aut_code),
    livre_id varchar(4) not null references livres(liv_code),
